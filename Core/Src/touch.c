...
// Định nghĩa delay_ms thành hàm delay của HAL
...
// ---------------------------------------------------------
// Hàm bọc giúp dễ dàng lấy tọa độ trực tiếp trong main.c
// ---------------------------------------------------------
uint8_t Get_Touch_XY(uint16_t *x, uint16_t *y)
{
    // Đọc trực tiếp trạng thái chân ngắt (IRQ)
    if (!(XPT2046_IRQ_READ()))
    {
        // Lấy thẳng giá trị thô (Raw) từ chip XPT2046
        if (xpt2046_twice_read_xy(x, y))
        {
            return 1; // Có chạm
        }
    }
...
{	
	...
	lcd_display_string(40, 40, (const uint8_t *)"Please use the stylus click the cross on the screen.", 16, RED);
		...
		if((s_tTouch.chStatus & 0xC0) == TP_PRESSED) {	
			...
			switch(cnt) {			   
				case 1:						 
					...
					tp_draw_touch_point(240 - 20, 20, RED); // Giả sử LCD_WIDTH = 240
					break;
				case 2:
					tp_draw_touch_point(240 - 20, 20, WHITE);
					tp_draw_touch_point(20, 320 - 20, RED); // Giả sử LCD_HEIGHT = 320
					break;
				case 3:
					tp_draw_touch_point(20, 320 - 20, WHITE);
					tp_draw_touch_point(240 - 20, 320 - 20, RED);
					break;
				case 4:	
					tem1=abs((int16_t)(pos_temp[0][0]-pos_temp[1][0]));
					tem2=abs((int16_t)(pos_temp[0][1]-pos_temp[1][1]));
					tem1*=tem1; tem2*=tem2; tem1+=tem2; d1=sqrt(tem1);

					tem1=abs((int16_t)(pos_temp[2][0]-pos_temp[3][0]));
					tem2=abs((int16_t)(pos_temp[2][1]-pos_temp[3][1]));
					tem1*=tem1; tem2*=tem2; tem1+=tem2; d2=sqrt(tem1);
					fac=(float)d1/d2;
					if(fac<0.95||fac>1.05||d1==0||d2==0) {
						cnt=0;
						delay_ms(1000);
						lcd_fill_rect(96, 240, 24, 16, WHITE);
						tp_draw_touch_point(240 - 20, 320 - 20, WHITE);
						tp_draw_touch_point(20, 20, RED);
						continue;
					}

					s_tTouch.fXfac = (float)(240 - 40) / (int16_t)(pos_temp[1][0] - pos_temp[0][0]);	
					s_tTouch.iXoff = (240 - s_tTouch.fXfac * (pos_temp[1][0] + pos_temp[0][0])) / 2;

					s_tTouch.fYfac = (float)(320 - 40) / (int16_t)(pos_temp[2][1] - pos_temp[0][1]);	  
					s_tTouch.iYoff = (320 - s_tTouch.fYfac * (pos_temp[2][1] + pos_temp[0][1])) / 2;
